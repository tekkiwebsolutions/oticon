...
class HomeController extends Controller
{
    ...
    public function aboutHearing(Request $request)
    {
        if(isset($request->id) && $request->id>0){
            $about_hearing_file = DB::table('about_hearing as ah')
            ->select('ah.*')
            ->leftJoin('age_group as a', 'a.id', '=', 'ah.age_group_id')
            ->where([['a.slug', 'LIKE', $request->ageCat]])
            ->where([['ah.id', '=', $request->id]])
            ->first();
        ...
        if (isset($about_hearing_file->id) && $about_hearing_file->id > 0) {
            $about_hearing_popup = DB::table('about_hearing_popup')->where('about_hearing_id', $about_hearing_file->id)->get();
        } else {
            $about_hearing_file = [];
            $about_hearing_popup = [];
        }

        return view('about_hearing', ['ageCatUrl' => $request->ageCat, 'curruntpage' => 'about_hearing', 'about_hearing_popup' => $about_hearing_popup, 'about_hearing_file' => $about_hearing_file]);
    }
    ...
    public function mediaupload(Request $request)
    {
        ...
        $this->validate($request, [ 
            'media_name' =>'required|file|mimes:audio/mpeg,mpga,mp3,wav,aac'
        ]);  
    ...
    public function save_xls(Request $request)
    {
        ...
        $data = Saved_styles::create([
            ...
            'styles_product_id' => $request->styles_product_id,
