                        ...
                        <form method="POST" action="{{ url('save_agendas') }}" novalidate>
                            ...
                            <div class='agenda-form'>
                                <div class='agenda-box-shadow'>
                                    <input type="input" class="form-controls" placeholder="Agenda Name" name="title" value="@if(isset($data->title)){{$data->title}}@endif">
                                    @if($errors->has('title'))<span style="color:red;">
                                        <div class="error">{{ $errors->first('title') }}</div>
                                    </span>@endif


                                    <input type="input" class="form-controls" placeholder="Client Name" name="client_name" value="@if(isset($data->client_name)){{$data->client_name}}@endif">
                                    @if($errors->has('client_name'))<span style="color:red;">
                                    ...
                                    @if($errors->has('description'))<span style="color:red;">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-5">
                                                <select name="selections[]" id="multiselect1" class="form-controls" size="8" multiple="multiple">
                                                    <option value="Introduction" @if(in_array('Introduction',$sectionss)) selected @endif>Introduction</option>
                                                    <option value="About Hearing" @if(in_array('About Hearing',$sectionss)) selected @endif>About Hearing</option>
                                                    <option value="Your Hearing" @if(in_array('Your Hearing',$sectionss)) selected @endif>Your Hearing</option>
                                                    <option value="Situations" @if(in_array('Situations',$sectionss)) selected @endif>Situations</option>
                                                    <option value="Your Solution" @if(in_array('Your Solution',$sectionss)) selected @endif>Your Solution</option>
                                                    <option value="Aural Rehab" @if(in_array('Aural Rehab',$sectionss)) selected @endif>Aural Rehab</option>
                                                </select>
                                            </div>
                                            @if($errors->has('selections'))<span style="color:red;"><div class="error">{{ $errors->first('selections') }}</div></span>@endif
                                        </div>
                                    </div>
                                </div>
                                <div class='row'>
